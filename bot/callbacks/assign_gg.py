...
async def handle_user_selection(callback: types.CallbackQuery, state: FSMContext):
    print("✅ Assign GG (select) callback fired")
    telegram_id = int(callback.data.split(":")[2])
    display_name = callback.data.split(":")[3]
...
async def handle_cancel(callback: types.CallbackQuery, state: FSMContext):
    print("✅ Assign GG (cancel) callback fired")
    await state.clear()
...
async def handle_confirm(callback: types.CallbackQuery, state: FSMContext):
    print("✅ Assign GG (confirm) callback fired")
    telegram_id = int(callback.data.split(":")[2])
    ...
    await state.clear()
    response = await callback.message.edit_text("The deed is done. Gossip Girl has been chosen. XOXO 💋")
    asyncio.create_task(delete_with_delay([response],5))
